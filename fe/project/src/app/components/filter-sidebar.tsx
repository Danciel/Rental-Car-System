...
import { carTypeApi, carBrandApi, CarTypeResponse, CarBrandResponse } from '@/app/api/api';
...
export function FilterSidebar({ onFilterChange }: FilterSidebarProps) {
  ...
  const [carTypes, setCarTypes] = useState<CarTypeResponse[]>([]);
  const [brands, setBrands] = useState<CarBrandResponse[]>([]);
  ...
  useEffect(() => {
    carTypeApi.getAll()
      .then(setCarTypes)
      .catch(() => console.error('Failed to load car types'))
  ...
  const handleCarTypeToggle = (typeName: string) => {
    const newCarTypes = filters.carTypes.includes(typeName)
      ? filters.carTypes.filter(t => t !== typeName)
      : [...filters.carTypes, typeName];
  ...
  const handleBrandToggle = (brandName: string) => {
    const newBrandNames = filters.brandNames.includes(brandName)
      ? filters.brandNames.filter(b => b !== brandName)
      : [...filters.brandNames, brandName];
    const newFilters = { ...filters, brandNames: newBrandNames };
    setFilters(newFilters);
    onFilterChange(newFilters);
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 max-h-[calc(100vh-7rem)] overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-semibold text-gray-900">Filters</h3>
        <button onClick={handleReset} className="text-sm text-[#1E40AF] hover:underline">
          Reset All
        </button>
      </div>
          ...
          <div className="space-y-2">
            {carTypes.map((type) => (
              <label key={type.id} className="flex items-center gap-2 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={filters.carTypes.includes(type.typeName)}
                  onChange={() => handleCarTypeToggle(type.typeName)}
                  className="w-4 h-4 rounded border-gray-300 text-[#1E40AF] focus:ring-[#1E40AF]"
                />
                <span className="text-gray-700 group-hover:text-gray-900">{type.typeName}</span>
              </label>
            ))}
          </div>
        )}
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {brands.map((brand) => (
              <label key={brand.id} className="flex items-center gap-2 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={filters.brandNames.includes(brand.name)}
        ...
        <div className="space-y-4">
          <div>
            <label className="text-sm text-gray-600 mb-1 block">
              Min: {formatVND(filters.priceRange[0])}/ngày
            </label>
            <input
          ...
          <div>
            <label className="text-sm text-gray-600 mb-1 block">
              Max: {formatVND(filters.priceRange[1])}/ngày
            </label>
            <input
              type="range" min="0" max="3000000" step="100000"
